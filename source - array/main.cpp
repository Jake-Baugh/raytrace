...
HRESULT Init()
{
	...
	g_DeviceContext->Map(g_PrimitivesBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &PrimitivesResources); // Map it so it cannot be used by the grapchiscard, also means we have access on processor
	CustomStruct::Primitive l_primitive;


	l_primitive.Circle.MidPosition		= D3DXVECTOR4(0.0f, 0.0f, 700.0f, 1.0f);
	//l_primitive.Circle[0].Radius		= 200.0f;
	//l_primitive.Circle[0].Color		= D3DXVECTOR4(1.0f, 0.0f, 0.0f, 200.0f);
	
	/*
	l_primitive.Circle[1].MidPosition		= D3DXVECTOR4 (0.0f, 0.0f, 1500.0f, 1.0f);
	//l_primitive.Circle[1].Radius			= 200.0f;
	l_primitive.Circle[1].Color				= D3DXVECTOR4(1.0f, 1.0f, 0.0f, 200.0f);
	*/
	...
	l_primitive.Triangle[0].Position2		= D3DXVECTOR4(400.0f,  401.0f,  800.0f,  1.0f);
	l_primitive.Triangle[0].Position3		= D3DXVECTOR4(400.0f, -200.0f, 1002.0f,  1.0f);
	
	
	l_primitive.Triangle[1].Color			= D3DXVECTOR4(0.0f,		1.0f,	0.0f,	 1.0f);
	l_primitive.Triangle[1].Position1		= D3DXVECTOR4(1001.0f, -200.0f, 500.0f,  1.0f);
	l_primitive.Triangle[1].Position2		= D3DXVECTOR4(1000.0f,  400.0f, 800.0f,  1.0f);
	l_primitive.Triangle[1].Position3		= D3DXVECTOR4(1000.0f, -200.0f, 1001.0f, 1.0f);
	*/
	
	
	/*
	l_primitive.Amount.x = 2.0f;
	l_primitive.Amount.y = 2.0f;
	l_primitive.Amount.z = -1.0f;
	l_primitive.Amount.w = -1.0f;
	*/

	*(CustomStruct::Primitive*)PrimitivesResources.pData = l_primitive;
	g_DeviceContext->Unmap(g_PrimitivesBuffer, 0);	// Unmap from CPU, thus giving back access to GPU
...
HRESULT Update(float deltaTime)
{
	...
	l_eachFrameData.screenVariable.x	= 800.0f;
	l_eachFrameData.screenVariable.y	= 800.0f;
	l_eachFrameData.screenVariable.z	= -1.0f;
	l_eachFrameData.screenVariable.w	= -1.0f;	
